import { calcularBitsLineaYConjunto, CalcularTAGCache, CalcularTAGVictimCache, esPotenciaDe2, obtenerNumeroBitsParaNConjuntos, rangoBits } from "./calculo_bits";
...
export default class SimulatedCache {
    ...
    constructor(numeroLineas: number, tamanoLineas: number, asociatividad: number,
        ...
        if (numeroLineasVC > 0)
            this.victimCache = new SimulatedCache(numeroLineasVC, tamanoLineas, numeroLineasVC, 0, this.TAMANO_DIRECCION);
    }
    ...
    ObtenerLineaDeDireccion(direccion: number): LineaCache | null {
        const TAG = CalcularTAGCache(direccion, this.bitMaxLinea, this.bitMaxConjunto, this.conjuntos.length, this.TAMANO_DIRECCION);
    ...
    private ObtenerLineaDeVictimCache(direccion: number, conjunto: ConjuntoCache, TAG: number): LineaCache | null {
        if (!this.victimCache) return null;

        const TAGVictimCache = CalcularTAGVictimCache(TAG, conjunto.id, this.conjuntos.length);
        const conjuntoVictimCache = this.victimCache.ObtenerConjuntoDeDireccion(direccion);
        const lineaVictimCache = conjuntoVictimCache.BuscarLinea(TAGVictimCache);

        if (lineaVictimCache)
            return this.HitEnVictimCache(conjunto, TAG, conjuntoVictimCache, lineaVictimCache);

        return this.MissEnVictimCache(conjunto, TAG, conjuntoVictimCache);
    }

    private HitEnVictimCache(conjunto: ConjuntoCache, TAG: number, conjuntoVictimCache: ConjuntoCache, lineaVictimCache: LineaCache): LineaCache {
        this.hits++;
        ...
        if (lineaMasAntigua.activa) {
            conjuntoVictimCache.SobrescribirLinea(
                CalcularTAGVictimCache(lineaMasAntigua.TAG, conjunto.id, this.conjuntos.length),
                lineaVictimCache);
    ...
    private MissEnVictimCache(conjunto: ConjuntoCache, TAG: number, conjuntoVictimCache: ConjuntoCache): null {
        this.misses++;

        const lineaMasAntigua = conjunto.ObtenerLineaMasAntigua();

        if (lineaMasAntigua.activa) {
            const lineaMasAntiguaVictimCache = conjuntoVictimCache.ObtenerLineaMasAntigua();
            conjuntoVictimCache.SobrescribirLinea(
                CalcularTAGVictimCache(lineaMasAntigua.TAG, conjunto.id, this.conjuntos.length),
